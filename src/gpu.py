...
class GpuProcessor:
    ...
    def image_to_gpu(self, image: np.ndarray, h, w):
        # RGBA32F
    ...
    def lut_1d_to_gpu(self, lut: np.ndarray):

        xp = lut[0]
        fp = lut[1:]

        channels, size = fp.shape

        tex_data = np.zeros((channels, size, 4), dtype=np.float32)

        # store fp in red channel
        tex_data[..., 0] = fp
    ...
    def apply_lut_3d_shader(
        ...
        param_buffer = self.device.create_buffer_with_data(
            data=params.tobytes(),
            usage=wgpu.BufferUsage.UNIFORM,
        )
